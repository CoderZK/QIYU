...
#import "QQYYPhotoCell.h"
@interface QQYYMinePhotoTVC ()<TZImagePickerControllerDelegate,UINavigationBarDelegate,QQYYPhotoCellDelegate>
...
@implementation QQYYMinePhotoTVC
...
- (void)viewDidLoad {
    ...
    [self.tableView registerClass:[QQYYPhotoCell class] forCellReuseIdentifier:@"cell"];
    ...
    UIButton * newClickUpAndInsideBT=[[UIButton alloc] initWithFrame:CGRectMake(ScreenW - 45 - 15,  sstatusHeight + 2,45, 40)];
    ...
    self.rightBt = newClickUpAndInsideBT;
    ...
    if (self.photos.length > 0) {
        ...
        [self leftOrRightClickAction:newClickUpAndInsideBT];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    QQYYPhotoCell * cell =[tableView dequeueReusableCellWithIdentifier:@"cell" forIndexPath:indexPath];
...
- (void)gengXinPhotosWithString:(NSString *)str {
    ...
    [QQYYRequestTool networkingPOST:[QQYYURLDefineTool uploadPhotoURL] parameters:str success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        if ([responseObject[@"code"] intValue]== 0) {
            ...
            if (self.photos.length == 0) {
                [self leftOrRightClickAction:self.rightBt];
            }
